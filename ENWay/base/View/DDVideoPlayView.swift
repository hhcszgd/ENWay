...
class DDVideoPlayView: UIView {
    ...
    func addBoundaryTimeObserver() {
        ...
        let assetDuration = self.playerLayer?.player?.currentItem?.duration ?? CMTime.zero
        var times = [NSValue]()
...
extension DDVideoPlayView : DDPlayControlDelegate{
    func screenRotate(isFullScreen: Bool) {
        if isFullScreen {
...
protocol DDPlayControlDelegate : NSObjectProtocol{
    func screenChanged(isFullScreen:Bool)
    func sliderChanged(sender:DDVideoPlayView.DDSlider)
    func pressToPlay()
    func pressToPause()
}

extension DDVideoPlayView {
...
//    }
    class DDPlayerControlBar: UIView {
        ...
        private var tapCount : Int = 0
        weak var delegate : DDPlayControlDelegate?
        ...
        func _addsubViews()  {
            ...
            fullScreenButton.setTitle("全屏", for: UIControl.State.normal)//full screen
            fullScreenButton.setTitle("小屏", for: UIControl.State.selected)//not full screen
        ...
        @objc func sliderChanged(sender:DDVideoPlayView.DDSlider){
            performDelayHidden()
